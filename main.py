...
def novaVenda(): #minha função
  ...
  print("Valor total R$ %.2f reais" % valor_total)
...
def relatorio():
  ...
  print("\t+ Sal: %.0f unidades = R$ %.2f reais" % (sal_qtd,sal_qtd*PAO_SAL))
  print("\t+ Leite: %.0f unidades = R$ %.2f reais" % (leite_qtd, leite_qtd*PAO_LEITE))
  print("\t+ Milho: %.0f unidades = R$ %.2f reais" % (milho_qtd,milho_qtd*PAO_MILHO))

  #print("\nMais vendidos: ")

  v_total = sal_qtd*PAO_SAL + leite_qtd*PAO_LEITE + milho_qtd*PAO_MILHO

  print("\nValor total do dia: R$ %.2f reais" % v_total)
...
while(True): # loop infinito  
  ...
  if (opcao == "1"): # NOVA VENDA 
    ...
    os.system('clear') #limpa a tela
    print("Desenvolvido por mim, Rafael Tosta")
    time.sleep(10)
    os.system('clear') #limpa a tela
  elif(opcao == "4"):
    os.system('clear') #limpa a tela
